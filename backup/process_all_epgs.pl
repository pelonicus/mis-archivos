...
open(my $total, ">", "total_epgshare.xml") or die "No puedo escribir total_epgshare.xml\n";

print $total qq{<?xml version="1.0" encoding="UTF-8"?>\n};
print $total qq{<!DOCTYPE tv SYSTEM "xmltv.dtd">\n};
print $total qq{<tv>\n};
...
foreach my $gz_file (@gz_files) {
    ...
    if ($xml =~ /<tv[^>]*>(.*)<\/tv>/is) {
        ...
        # CHANNEL en una línea
        while ($content =~ /(<channel\b.*?<\/channel>)/gis) {
            my $x = $1;
            $x =~ s/\s+/ /g;
            print $total "$x\n";
        }

        # PROGRAMME en una línea
        while ($content =~ /(<programme\b.*?<\/programme>)/gis) {
            my $x = $1;
            $x =~ s/\s+/ /g;
            print $total "$x\n";
        }
